...
import { 
  Share2, ArrowLeft, Loader2, AlertCircle, Briefcase, 
  User, Mail, Phone, Globe, Award, Code, Star, 
  Copy, ExternalLink, Lock, Unlock, Sparkles, Zap, Target,
  GraduationCap, FolderOpen, Languages, Wrench, ChevronRight,
  Check, X, Twitter, Linkedin, Facebook, Link2, QrCode, LogIn, Heart
} from 'lucide-react';
import { 
  ...
  const shareOptions = [
    {
      ...
      url: `https://twitter.com/intent/tweet?text=Check out this amazing portfolio!&url=${encodeURIComponent(shareUrl)}`
    },
    {
      name: 'LinkedIn',
      icon: Linkedin,
              ...
              <div className="p-6 space-y-6">
                ...
                >
                  ...
                  <p className="text-sm text-purple-300">
                    Share this amazing portfolio with your network!
                  </p>
                </motion.div>
              </div>
            </div>
...
// Bento Card Component (same as before)
...
export default function PublicPortfolioPage({ params }) {
 ...
 const { identifier } = React.use(params);
  ...
  // ✅ Like mutation
  const [toggleLike] = useToggleLikePortfolioMutation();

  // ✅ Try fetching by ID first
  const { data: byId, isLoading: loadingId, error: errorId } = useGetPortfolioByIdQuery(identifier, {
    skip: identifier?.length !== 24
  });
  
  // ✅ Try fetching by slug if ID doesn't work
  const { data: bySlug, isLoading: loadingSlug, error: errorSlug } = useGetPortfolioBySlugQuery(identifier, {
    skip: identifier?.length === 24
  });

  const isLoading = loadingId || loadingSlug;
  const error = errorId || errorSlug;

  useEffect(() => {
    const portfolioData = byId?.portfolio || bySlug?.portfolio;
    if (portfolioData) {
      setPortfolio(portfolioData);
      setLikeCount(portfolioData.likes || 0);
      // Check if current user has liked this portfolio
      if (user && portfolioData.likedBy?.includes(user._id)) {
        setIsLiked(true);
      }
    }
  }, [byId, bySlug, user]);
  ...
  // ✅ Handle Like/Unlike
// ✅ Handle Like/Unlike - NO AUTHENTICATION REQUIRED
const handleLike = async () => {
  ...
  try {
    ...
    // Show feedback
    if (newLikeState) {
      toast.success('Portfolio liked! ❤️', {
        style: { 
          background: '#1a1a1a', 
          color: '#ceea45', 
          border: '1px solid rgba(206,234,69,0.3)',
      ...
      <div className="min-h-screen bg-black flex items-center justify-center">
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-gray-500 text-sm">Preparing showcase...</p>
          </div>
        </motion.div>
      </div>
    );
  ...
  // Error State - Portfolio Not Found
  if (error || !portfolio) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        ...
        >
          ...
          <h1 className="text-3xl font-black text-white mb-3">Portfolio Not Found</h1>
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        ...
        >
          <motion.div
            ...
            className="inline-block mb-6"
          >
            <div className="p-8 rounded-3xl bg-gradient-to-br from-yellow-500/20 to-orange-500/10 border border-yellow-500/20">
              <Lock size={56} className="text-yellow-400" />
            </div>
          </motion.div>
          <h1 className="text-3xl font-black text-white mb-3">Portfolio is Private</h1>
          <p className="text-gray-400 mb-8 leading-relaxed">
            This portfolio is not available for public viewing. The owner has set it to private.
          </p>
          <motion.button
            onClick={() => router.push('/Pages/Main/home')}
            className="px-8 py-4 bg-gradient-to-r from-[#ceea45] to-[#a8c930] text-black font-bold rounded-2xl hover:shadow-2xl hover:shadow-[#ceea45]/30 transition-all duration-300"
...
<motion.button
  ...
  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm transition-all duration-300 ${
    isLiked
      ...
      : 'bg-white/5 text-gray-300 border border-white/10 hover:bg-white/10 hover:border-red-500/30'
  }`}
  whileHover={{ scale: 1.05 }}
  whileTap={{ scale: 0.95 }}
>
  ...
  {isLiking && <Loader2 size={14} className="animate-spin" />}
</motion.button>
            {/* Quick Copy Button */}
            <motion.button
              onClick={quickCopy}
            ...
            {/* ✅ Connect Button - Login Required */}
            <motion.button
              onClick={() => router.push('/Pages/Auth/login')}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold hover:shadow-lg hover:shadow-blue-500/30 transition-all duration-300 text-sm"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <LogIn size={16} />
        ...
        {/* Bento Grid Layout - Same as before but READ ONLY */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
          
          {/* Profile Card - WITH LIKE COUNT DISPLAY */}
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <motion.div 
                    ...
                    whileHover={{ scale: 1.05 }}
                  >
                    <p className="text-2xl font-black text-red-400">{likeCount}</p>
        ...
        {/* ✅ Footer Actions - NO EDIT, ONLY SHARE & CONNECT */}
        <motion.footer
        ...
        >
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            ...
            <motion.button
              onClick={() => router.push('/Pages/Auth/login')}
              className="w-full sm:w-auto px-8 py-4 rounded-2xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold transition-all duration-300 flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-blue-500/30"
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
            >
              <LogIn size={18} />
          ...
          <motion.p 
            ...
            transition={{ delay: 1 }}
          >
            👥 Public Portfolio • 🔒 Read Only • 🌍 Share Anywhere • ❤️ Like & Support
          </motion.p>
        </motion.footer>
      </div>
